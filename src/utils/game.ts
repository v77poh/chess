...
export const getFigureWithDefaultMoves = ({squares, figure}:Pick<Movement, 'figure' | 'squares'>):Figure => {
        ...
        return defaultMovesEntries.reduce((acc, [direction, moves]) => {
            return {
                ...
                [direction]: moves.filter(move => {
                    ...
                    if(figure.type === Type.BLACK) {
                        isFirstMove = figure.position.y === 1;
                        const figureMoveForward = figuresMap[getKey({y: figure.position.y + 1, x: figure.position.x})]
                        const figureMoveForward2 = figuresMap[getKey({y: figure.position.y + 2, x: figure.position.x})]
                        const figureRight = figuresMap[getKey({y: figure.position.y + 1, x: figure.position.x + 1})]
                        const figureLeft = figuresMap[getKey({y: figure.position.y + 1, x: figure.position.x - 1})]
                        right = figure.position.y + 1 === movePosition.y &&
                                figure.position.x + 1 === movePosition.x &&
                                !!figureRight &&
                                figureRight.type !== Type.BLACK;
                        left = figure.position.y + 1 === movePosition.y &&
                                figure.position.x - 1 === movePosition.x &&
                                !!figureLeft &&
                                figureLeft.type !== Type.BLACK;
                        moveForward =
                            figure.position.y + 1 === movePosition.y &&
                            figure.position.x === movePosition.x &&
                            !figureMoveForward;

                        moveForward2 = figure.position.y + 2 === movePosition.y &&
                            figure.position.x === movePosition.x &&
                            !figureMoveForward &&
                            !figureMoveForward2
                        ...
                        isFirstMove = figure.position.y === 6;
                        moveForward =
                            figure.position.y - 1 === movePosition.y &&
                            figure.position.x === movePosition.x &&
                            !figureMoveForward
                        ...
                        moveForward2 =
                            figure.position.y - 2 === movePosition.y &&
                            figure.position.x === movePosition.x &&
                            !figureMoveForward &&
                            !figureMoveForward2
                    }
...
export const isEnPassant = (figure:Figure, toPosition:Position, history:History[]) => {
    ...
    if(lastMoved && lastMoved.figure.piece === Piece.PAWN) {
        if(figure.piece === Piece.PAWN) {
            const isPawnLeftOrRight = Math.abs(lastMoved.from.x - figure.position.x) === 1;
            ...
            if( figure.type === Type.BLACK ) {
                if(figure.position.y === 4) {
                    if(
                        ...
                        isPawnLeftOrRight &&
                        toPosition.x === lastMoved.from.x &&
                        toPosition.y === lastMoved.from.y - 1
                    ){
                        return true
                    }
                }
            } else {
                if(figure.position.y === 3) {
                    if(
...
const movePawn = ({position, toPosition}:MovementProps) => {
    ...
    return  (Math.abs(toY) === 1 && Math.abs(toX) === 1) ||
            ( (Math.abs(toY) === 1 || Math.abs(toY) === 2) && toX === 0)
}
...
export const getCheckedRows = (move:Type, figuresMap:FiguresMap):CheckedRows => {
    ...
    return pieces.reduce((acc, enemyFigure) => {
        ...
        if(attackRow && attackRow.length > 0) {
            ...
            console.log({rowInBetweenKing, attackRow})
            if(isCheck) {
                return {
                    ...acc,
